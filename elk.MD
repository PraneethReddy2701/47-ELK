...
filter {
  grok {
    match => {
      "message" => '%{IP:client_ip} \[%{HTTPDATE:timestamp}\] %{WORD:method} %{URIPATH:request} HTTP/%{NUMBER:http_version} %{NUMBER:status} %{NUMBER:bytes_sent} "%{DATA:referrer}" %{NUMBER:request_time}'
    }
  }
